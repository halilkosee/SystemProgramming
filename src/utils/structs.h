...
typedef struct client_command_argument{
  char* requestFileaPath;
  char* IP;
  int PORT;
  int BARRIER;
}client_command_argument;

typedef struct distributor_command_argument{
  char* directoryPath;
  char* IP;
  char* portAsString;
  int numberOfServant;
  int PORT;
}distributor_command_argument;

typedef struct servant_command_argument{
  char* directory_path;
  char* borders;
  char* IP;
  int PORT;
}servant_command_argument;

typedef struct server_command_argument{
  int PORT1;
  int PORT2;
  int numberOfThreads;
}server_command_argument;
