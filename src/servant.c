...
servant_command_argument arguments;
...
int main(int argc, char *argv[]) {
    getDataWithFifo();
    arguments.directory_path = argv[1];
    arguments.borders = argv[2];
    arguments.IP = argv[3];
    arguments.PORT = atoi(argv[4]);
  run();
  return 0;
}
...
void run() {
  writeOnTerminal("servant is running...\n");
    ...
    get_border(arguments.borders, &lower_bound, &upper_bound);
    
    char main_directory[MAX_FOLDER][FOLDER_NAME_LEN];
    folder responsible_directories[upper_bound - lower_bound + 1];

    list_data_folders(arguments.directory_path, main_directory, &main_folder_counter, DT_DIR);
    ...
    int client_fd = setup_client(arguments.IP, unique_port, &mutex);
    ...
    strcpy(req.ip_address, arguments.IP);
...
void get_sub_folders(folder resp_dir[], int num_folders, int* num_data){
    ...
    for(int i = 0; i < num_folders; i++){
        ...
        sprintf(path, "%s/%s", arguments.directory_path, resp_dir[i].name);
...
void get_data_from_dataset(folder resp_dir[], int number_of_resp_file, int number_of_transactions_file, City cities[]){
    ...
    for(int i=0; i<number_of_resp_file; i++){
        ...
        for(int j=0; j<number_of_transactions_file; j++){
            ...
            sprintf(path, "%s/%s/%s", arguments.directory_path, resp_dir[i].name, resp_dir[i].sub_names[j]);
