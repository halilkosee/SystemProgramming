...
server_command_argument arguments;
...
int main(int argc, char *argv[]) {
  int opt;
  while(-1 != (opt = getopt(argc, argv, "p:q:t:"))) {
    switch(opt) {
      case 'p':
        arguments.PORT1 = atoi(optarg);
        break;
      case 'q':
        arguments.PORT2 = atoi(optarg);
        break;
      case 't':
        arguments.numberOfThreads = atoi(optarg);
        break;
      default:
        displayUsage();
    }
...
void displayUsage() {               
  writeError(" Usage: ./server -p PORT1(must be numeric) -q PORT2(must be numeric) -t numberOfTHreads(must be numeric and higher than 4) \n");
  exit(EXIT_FAILURE);
}

void validateArguments(int argc) {
  if (argc != 7) {displayUsage();}
  if (!(arguments.PORT1) || !(arguments.PORT2) ) {displayUsage();}
  if (arguments.numberOfThreads < 4) {displayUsage();}
}

void run() {
  writeOnTerminal("server is running...\n");
   struct sigaction sa;
    ...
    threads = (pthread_t*)calloc(sizeof(pthread_t), arguments.numberOfThreads);
    for(int i = 0; i < arguments.numberOfThreads; ++i){
        pthread_create(&threads[i], NULL, (void*) thread_main, head);
    }

    int server_fd = setup_server(arguments.PORT1, BACKLOG);
